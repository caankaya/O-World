...
import { Mars } from '../@types/planetDatas';
import { fadeIn } from '../utils/motion';

import SimpleLoader from './SimpleLoader';

function MarsInfos({ marsData }: { marsData: Mars }) {
  const infiniteLoadingInfos = useAppSelector(
    (state) => state.planet.infiniteLoading
  );

  if (infiniteLoadingInfos || Object.keys(marsData).length === 0) {
    return <SimpleLoader />;
  }
