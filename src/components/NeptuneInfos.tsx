...
import { Neptune } from '../@types/planetDatas';
import { fadeIn } from '../utils/motion';

import SimpleLoader from './SimpleLoader';

function NeptuneInfos({ neptuneData }: { neptuneData: Neptune }) {
  const infiniteLoadingInfos = useAppSelector(
    (state) => state.planet.infiniteLoading
  );

  if (infiniteLoadingInfos || Object.keys(neptuneData).length === 0) {
    return <SimpleLoader />;
  }
